...
then
    GOOS=windows
    go build -v -ldflags "-s -w -G" -o builds/asmr-downloader-${1}-amd64.exe
elif [ "${1}" == "osx" ]
then
    GOOS=darwin
    go build -v -ldflags "-s -w -G" -o builds/asmr-downloader-darwin-amd64
else
    GOOS=linux
    go build -v -ldflags "-s -w -G" -o builds/asmr-downloader-${1}-amd64
fi
